...
namespace TransportManagmentSystemAPI.Services
{
    public class ReservationManagementService : IReservationManagement
    {
        ...
        public List<ReservationManagement> DisplayAllReservation(string travallerId)
        {
            ...
            {
                index = index + 1;
                TrainManagement gottrain = _trainList.Find(tra => tra.Id == res.Train).ToList().FirstOrDefault();
        ...
        public Dictionary<int, string> CreateReservation(ReservationManagement reservation)
        {
            ...
            {
                ReservationManagement existingReservationforUpdate = _reservationList.Find(exres => exres.Id == reservation.Id).FirstOrDefault();
                TimeSpan timeDiff = existingReservationforUpdate.ReservationDate - DateToday;

                if (timeDiff.TotalDays < 5)
                {
                    returnCode.Add(100, "Your reservation is confirmed and cannot be updated or canceled.");
                    return returnCode;
                }
                else if (timeDifferenceWhenReserve.TotalDays < 0)
                {
                    returnCode.Add(100, "Invalid reservation date. Please choose a valid date.");
                    return returnCode;
                }
                else
                {
                    ...
                    returnCode.Add(400, "Reservation successfully updated.  " + reservation.Id);
                    return returnCode;
                }
            }
            else
            {
                ...
                {
                    returnCode.Add(100, "Reservations must be made within 30 days from the booking date.");
                    return returnCode;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        returnCode.Add(400, "Reservation successfully created. " + reservation.Id);
                        return returnCode;
        ...
        public Dictionary<int, string> CancelledReservation(string id, ReservationManagement reservation)
        {
            ...
            if (reservation != null)
            {
                ReservationManagement existingReservation = _reservationList.Find(exres => exres.Id == id).FirstOrDefault();
                TimeSpan timeDiff = existingReservation.ReservationDate - DateTime.Now;

                if (timeDiff.TotalDays >= 5)
                {
                    var Cancelled = Builders<ReservationManagement>.Update.
                          Set(res => res.IsCancelled, reservation.IsCancelled);

                    var updatedProfile = _reservationList.UpdateOne(reser => reser.Id == id, Cancelled);
                    cancellingStat.Add(100, "Reservation has been canceled. reference ID " + existingReservation.ReferenceId);
                    return cancellingStat;
                }
                else
                {
                    cancellingStat.Add(500, "Your reservation is confirmed and cannot be canceled.");
                    return cancellingStat;
                }
            }
            else
        ...
        public Dictionary<int, string> DeleteReservation(string id)
        {
            ...
            if (existingReservation != null)
            {
                ...
                if (timeDiff.TotalDays >= 5)
                {
                    ...
                    {
                        deletionStatus.Add(500, "Failed to delete reservation");
                    }
                }
                else
                {
                    deletionStatus.Add(400, "Cannot delete the reservation as it is confirmed or less than 5 days before the reservation date.");
                }
            }
            else
            {
