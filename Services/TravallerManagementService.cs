...
namespace TransportManagmentSystemAPI.Services
{
    public class TravallerManagementService : ITravallerManagementService
    {
        ...
        public TravallerManagementService(IDatabaseSettings _databaseSettings, IScheam _scheam)
        {
            ...
            _travalerProfileList = database.GetCollection<TravallerManagement>(
                _scheam.TravellerScheama
            );
            _userList = database.GetCollection<UserManagement>(_scheam.UsersScheama);
        }

        // update and create the traveler account
        public TravallerManagement CreateAndUpdateTravellerAccount(
            TravallerManagement travallerProfile
        )
        {
            try
            {
                if (travallerProfile.Id != null)
                {
                    ...
                    var updatedProfile = _travalerProfileList.UpdateOne(
                        trav => trav.Id == travallerProfile.Id,
                        update
                    );

                    if (
                        travallerProfile?.UserInfo != null
                        && travallerProfile.UserInfo.Password != null
                    )
                    {
                        var updatePassword = Builders<UserManagement>.Update.Set(
                            upf => upf.Password,
                            travallerProfile.UserInfo.Password
                        );
                        var passwordReset = _userList.UpdateOne(
                            up => up.Nic == travallerProfile.Nic,
                            updatePassword
                        );
                    }
                ...
                else
                {
                    ...
                    var uniqueCounts = _travalerProfileList
                        .Find(trv => trv.Nic == travallerProfile.Nic)
                        .ToList()
                        .Count;
                    if (uniqueCounts == 0)
                    {
                        travallerProfile.CreatedDate = DateTime.Now;
                        // Initialize UserInfo if it's null
                        if (travallerProfile.UserInfo == null)
                        {
                            travallerProfile.UserInfo = new UserManagement();
                        }
        ...
        {
            var profile = _travalerProfileList
                .Find(pro => pro.Nic == _Nic)
                .ToList()
                .FirstOrDefault();
            if (profile != null)
            {
        ...
        public List<TravallerManagement> DisplayAllActiveAccounts(bool isActive)
        {
            try
            {
                var profileList = _travalerProfileList
                    .Find(trav => trav.AccStatus == isActive)
                    .ToList();

                List<TravallerManagement> secureProfileList = profileList
                    .Select(item =>
                    {
                        // item.UserInfo = null;
                        return item;
        ...
        public TravallerManagement ManageActivationTravellerAccountDetails(
            string nic,
            TravallerManagement travallerProfile
        )
        {
            try
            {
                if (nic != null)
                {
                    var updatedStatus = Builders<TravallerManagement>.Update.Set(
                        upf => upf.AccStatus,
                        travallerProfile.AccStatus
                    );
